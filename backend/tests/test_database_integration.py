...
class DatabaseIntegrationTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.postgres_dsn = os.getenv("POSTGRES_DSN", "").strip()
    ...
    def test_vector_search_roundtrip(self) -> None:
        if not (os.getenv("STEP_API_KEY", "").strip() or os.getenv("DASHSCOPE_API_KEY", "").strip()):
            self.skipTest("STEP_API_KEY/DASHSCOPE_API_KEY is not configured")
