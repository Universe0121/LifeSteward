...
class QueueLLMService(LLMService):
    def __init__(self, responses: list[str]) -> None:
        ...
        self.fallback_response = responses[-1] if responses else ""

    def generate(self, prompt: str, variables: Mapping[str, Any]) -> str:
        self.calls.append(dict(variables))
        if not self.responses:
            # Planning and interaction are separate LLM stages.  Reuse the
            # last deterministic fixture response when a routing test only
            # supplies one user-facing response.
            return self.fallback_response
            """
            return self.calls and "鏀跺埌銆? or ""
        """
...
class MasterAgentRoutingTest(unittest.TestCase):
    def test_planning_route_is_memory_then_planning(self) -> None:
        agent = MasterAgent(memory_service=InMemoryMemoryService())
        handlers = agent._intent_handlers["planning"]
        self.assertEqual(
            [handler.__self__.__class__.__name__ for handler in handlers],
            ["MemoryAgent", "PlanningAgent"],
        )
