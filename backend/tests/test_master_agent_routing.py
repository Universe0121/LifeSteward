...
class MasterAgentRoutingTest(unittest.TestCase):
    ...
    def test_planning_keeps_memory_then_interaction_route(self) -> None:
        # The queue supplies PlanningAgent output followed by InteractionAgent output.
        llm = QueueLLMService([
            json.dumps([{"task_name": "study", "start_time": "09:00", "duration_minutes": 60, "difficulty": 0.5}]),
            "planning response",
        ])
        class RoutingMemoryService(InMemoryMemoryService):
            def search_memory(self, user_id: str, memory_query: str, top_k: int = 5):
                self.search_calls.append({"user_id": user_id, "memory_query": memory_query, "top_k": top_k})
                return [{"event_content": "memory context"}]

        memory = RoutingMemoryService()
        result = MasterAgent(llm_service=llm, memory_service=memory).process(
            create_state("planning", "鏍规嵁瀛︿範鍘嗗彶鍒跺畾璁″垝")
        )
        self.assertEqual(result["assistant_response"], "planning response")
        self.assertEqual(len(result["retrieved_memories"]), 1)
        self.assertEqual(len(memory.search_calls), 1)
        self.assertEqual(result["generated_plan"], [{"task_name": "study", "start_time": "09:00", "duration_minutes": 60, "difficulty": 0.5}])
        self.assertEqual(len(llm.calls), 2)
        self.assertEqual(llm.calls[0]["retrieved_memories"], result["retrieved_memories"])
        self.assertEqual(llm.calls[1]["generated_plan"], result["generated_plan"])
        """
