...
### 50-record simulation dataset (direct initialization)

This dataset is deliberately initialized outside the Agent flow. The script
inserts each event into PostgreSQL, calls the configured embedding provider,
and writes the resulting vector into `memories` through `VectorSearchTool`:

```powershell
cd backend
..\.venv\Scripts\python.exe scripts\seed_simulation_data.py --count 50
..\.venv\Scripts\python.exe -m uvicorn main:app --reload
```

Open `http://127.0.0.1:8000/simulation-demo`. The timeline reads the real
`GET /api/v1/life-events` endpoint and chat uses the production
`POST /api/v1/chat` endpoint. Re-running the seed command replaces only the
tagged `simulation_v1` records for `user_id=10001`; it never truncates tables.
