...
export default function TaskManagementScreen({ route, navigation }: Props) {
  ...
  const date_options = useMemo(() => {
    const values = Array.from({ length: 31 }, (_, index) => add_days(today, index));
    if (route.params?.task_date && !values.includes(route.params.task_date)) values.push(route.params.task_date);
    return [...new Set(values)].sort();
  }, [route.params?.task_date, today]);
  const consumed_edit_task_ref = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (!task_id || consumed_edit_task_ref.current === task_id) return;
    const task = data.tasks.find((item) => item.task_id === task_id);
    if (task) {
      consumed_edit_task_ref.current = task_id;
  ...
  function close_editor() {
    ...
    consumed_edit_task_ref.current = null;
