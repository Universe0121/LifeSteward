...
import WeeklyPosterPreview from '../components/WeeklyPosterPreview';
...
export default function WeeklyReportScreen({ route }: Props) {
  ...
  const total = data.activity_analysis?.total_events ?? (typeof data.stats?.total_events === 'number' ? data.stats.total_events : 0);
  const completion_rate = data.completion?.completion_rate ?? 0;

  async function share_poster() {
    setSharing(true);
    setError('');
    let file_uri: string | null = null;
    try {
