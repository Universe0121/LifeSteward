...
export function useVoiceInput(user_id: number, onTranscribed: (text: string) => void) {
  ...
  const start_recording = useCallback(async () => {
    ...
    try {
      ...
      if (!permission.granted) { setState('error'); setErrorMessage('麦克风权限未开启，请在系统设置中允许后重试。'); return; }
      await Audio.setAudioModeAsync({ allowsRecordingIOS: true, playsInSilentModeIOS: true });
      const result = await Audio.Recording.createAsync(Audio.RecordingOptionsPresets.HIGH_QUALITY, (status) => {
        setDurationMs(status.durationMillis ?? 0);
      });
      recording_ref.current = result.recording;
      setDurationMs(0);
      setState('recording');
    } catch { setState('error'); setErrorMessage('录音启动失败，请保留文字输入或稍后重试。'); }
  }, []);
  ...
  const stop_recording = useCallback(async () => {
    ...
    try {
      ...
      setState('idle');
    } catch { setState('error'); setErrorMessage('语音转写暂时不可用，请直接输入文字或稍后重试。'); }
  }, [onTranscribed, user_id]);
