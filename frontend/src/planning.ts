...
const past_record_words = /(我(今天|昨天|刚刚|刚才)?(已经|刚刚|刚才)?(完成了|做完了|睡了|吃了|学习了|运动了)|完成了|做完了|睡了|吃了|学习了|运动了)/;
...
export function is_task_only_request(value: string): boolean {
  ...
  return date_words.test(text) && (task_action_words.test(text) || common_task_verbs.test(text)) && !past_record_words.test(text);
}
...
export function normalize_plan_items(value: unknown): PlanItem[] {
  ...
  return value.flatMap((item) => {
    ...
    const duration_minutes = Number(candidate.duration_minutes);
    const difficulty = Number(candidate.difficulty);
    if (!task_name || !/^([01]\d|2[0-3]):[0-5]\d$/.test(start_time) || !Number.isFinite(duration_minutes) || duration_minutes <= 0) return [];
    return [{ task_name, start_time, duration_minutes: Math.trunc(duration_minutes), difficulty: Number.isFinite(difficulty) ? Math.max(0, Math.min(1, difficulty)) : 0.5 }];
  });
}
...
export function classify_chat_action(user_input: string, response?: ChatResponse): ChatAction {
  ...
  if (normalize_plan_items(response?.generated_plan).length > 0 || has_explicit_clock(text) || plan_words.test(text) || response?.intent === "planning") return "plan";
  return "none";
}

export function requested_date_key(value: string, base_date = new Date()): string {
  const date = new Date(base_date);
  if (/后天/.test(value)) date.setDate(date.getDate() + 2);
  else if (/明天|次日/.test(value)) date.setDate(date.getDate() + 1);
  else if (/昨天/.test(value)) date.setDate(date.getDate() - 1);
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
}

export function extract_task_name(value: string): string {
  let text = value.trim().replace(/^\s*(请|麻烦|帮我|请你)?\s*/, "");
